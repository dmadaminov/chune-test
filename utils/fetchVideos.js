...
const fetchVids = (name, channelId) => {
    ...
    return axios.get(url)
        .then(matches => {
            ...
            matches.data.items.forEach(match => {
                if (match.snippet.title.toLowerCase().indexOf(name)>=0) result.push({ artist: name, url: match.id.videoId, title: match.snippet.title  })
            })
            return result
    })
}
